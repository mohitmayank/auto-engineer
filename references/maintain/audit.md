<!-- Part of the Auto Engineer skill's MAINTAIN specialization. Load this file when running the AUDIT sub-mode. -->
...
> **Authorized defensive use.** This sub-mode audits the user's *own* repository to find
...
Distinct from the built-in **`/security-review`** (reviews the *pending diff* on the
current branch) — AUDIT scans the **whole committed repo**, deps included. They complement.
...
- **Dep CVEs** → resolve via the smallest version move that clears it (delegate the actual
  bump mechanics to the UPGRADE sub-mode's per-ecosystem steps). Prefer patched minors;
...
- **MAINTAIN → UPGRADE** — does the actual version moves for vulnerable deps.
- **`/security-review`** (built-in) — pair with this to also cover the pending diff.
- **BUILD specialization** — if remediation is a real refactor (e.g. replacing an auth flow), hand off.

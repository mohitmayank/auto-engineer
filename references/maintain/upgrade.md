<!-- Part of the Auto Engineer skill's MAINTAIN specialization. Load this file when running the UPGRADE sub-mode. -->
...
**Not** for: adding a *new* dependency (that's a BUILD-specialization feature decision), or
auditing *vulnerabilities* specifically → use **MAINTAIN → AUDIT** (it triages CVEs;
UPGRADE moves versions).
...
- **MAINTAIN → AUDIT** — if the goal is fixing *vulnerabilities*, start there; it routes
  back here for the version moves.
- **MAINTAIN → DEFLAKE** — flaky tests can mask whether an upgrade truly passed.
- **BUILD specialization** — if an upgrade needs real feature-level migration work, hand off.

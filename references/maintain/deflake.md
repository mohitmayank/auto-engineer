<!-- Part of the Auto Engineer skill's MAINTAIN specialization. Load this file when running the DEFLAKE sub-mode. -->
...
(BUILD specialization for a fix, or just fix it directly). DEFLAKE targets *nondeterministic*
failures.
...
Establish flakiness empirically — a test isn't flaky because someone said so. Use a
configured repeat-run count as the default N for repeat-runs (else 20):
...
- **MAINTAIN → UPGRADE** — a flaky suite makes upgrade verification unreliable; deflake first.
- **MAINTAIN → DEBT** — flaky-test annotations are test debt; this clears them at the root.
- **BUILD specialization** — when the flake is a genuine product-code race needing real design.

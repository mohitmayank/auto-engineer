<!-- Part of the Auto Engineer skill. Load this file when the agent needs guidance on executing tasks in parallel waves, agent orchestration, prompt templates, and error recovery. -->
...
```
Agent 1: Execute AE-002 (Database schema)
Agent 2: Execute AE-003 (API router setup)
Agent 3: Execute AE-004 (Config files)
```
...
When a task is blocked:
  ...
  3. Record the blocking task ID (if applicable): {AE-XXX}
...
2. **Non-blocking discovery** (nice-to-have, related improvement, cleanup): Do NOT absorb it. Add it to a `## Deferred Work` section on the board. Mention it in the CONVERGE summary. The user decides whether to start a new Auto Engineer session for it.
...
Every Auto Engineer task graph must include these three tasks as the final tasks, in this order:

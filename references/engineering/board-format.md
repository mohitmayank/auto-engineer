<!-- Part of the Auto-Engineer AbsolutelySkilled skill. Load this file when the agent needs the full specification for the .auto-engineer/board.md file, including format, status transitions, and examples. -->

# Board Format Specification

The `.auto-engineer/board.md` file is the single source of truth for a Auto-Engineer execution. It tracks everything from intake through convergence and is designed to be both human-readable and machine-parseable.
...
```
{project-root}/.auto-engineer/board.md
```

The `.auto-engineer/` directory may also contain:
...
|----|-------|------|------|-------------|------|--------|
| AE-001 | {title} | code | S | - | 1 | done |
| AE-002 | {title} | code | M | AE-001 | 2 | in-progress |
| AE-003 | {title} | test | S | AE-002 | 3 | pending |
...
- **Wave 1** (N tasks): AE-001, AE-002 [parallel]
- **Wave 2** (N tasks): AE-003 [serial]
- **Wave 3** (N tasks): AE-004, AE-005, AE-006 [parallel]
```
...
- **Size**: S | M
- **Dependencies**: none | [AE-XXX, AE-YYY]
- **Wave**: {N}
...
When starting a new session and a `.auto-engineer/board.md` exists:
...
### Resume Detection
At the start of any Auto-Engineer invocation:
1. Check if `.auto-engineer/board.md` exists
2. If yes, ask the user: "Found an existing Auto-Engineer board. Resume it or start fresh?"
...
| ID | Title | Type | Size | Dependencies | Wave | Status |
|----|-------|------|------|-------------|------|--------|
| AE-001 | NextAuth config + providers | config | S | - | 1 | done |
| AE-002 | User + Account Prisma models | config | S | - | 1 | done |
| AE-003 | Auth API route handler | code | M | AE-001, AE-002 | 2 | in-progress |
| AE-004 | Auth middleware for protected routes | code | M | AE-001 | 2 | in-progress |
| AE-005 | Login page component | code | M | AE-003 | 3 | pending |
| AE-006 | Register page component | code | M | AE-003 | 3 | pending |
| AE-007 | Auth e2e tests | test | M | AE-005, AE-006 | 4 | pending |
| AE-008 | Auth API documentation | docs | S | AE-003 | 4 | pending |

### Wave Assignments

- **Wave 1** (2 tasks): AE-001, AE-002 [parallel] - COMPLETED
- **Wave 2** (2 tasks): AE-003, AE-004 [parallel] - IN PROGRESS
- **Wave 3** (2 tasks): AE-005, AE-006 [parallel] - PENDING
- **Wave 4** (2 tasks): AE-007, AE-008 [parallel] - PENDING

## Tasks

### AE-001: NextAuth config + providers
- **Type**: config
- **Size**: S
- **Dependencies**: none
- **Wave**: 1
